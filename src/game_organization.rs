...
pub fn algorithmic_trial_eval(trial_directory: &str, evaluations: u16, move_orders: u8, output_name: &str, a_b_depth: u8, time_cap:u8, dir: u8) {
    //Iterates over GameState files to hold matches between Algorithmic Players
    println!("Testing evaluations up to {} over the directory: {}", evaluations, trial_directory);
    let data_name = format!("../{}_test_result.txt", output_name);
    let file_path = PathBuf::from(trial_directory).join(data_name);
    ...
    for path in paths {
        if path.is_ok() {
            ...
            for algorithmic_pair in &eval_pairs{
                ...
                if let Ok(test_results) = trial_play_adj(&trial.path(), &tc, &time_cap) {
                    //Test concluded, write the returned data to our file
                    let new_entry = format!("{:?},{},{},{},{},{},{},{},{},{},{},{}\n", trial.file_name(), tc.a_b_depth, utility::store_vc(&test_results.victory), test_results.length, tc.attacker_eval, tc.attacker_mo, test_results.avg_attack_time, test_results.worst_attack_time, tc.defender_eval, tc.defender_mo, test_results.avg_defend_time, test_results.worst_defend_time);
                    if file.write(new_entry.as_bytes()).is_ok() {
                        println!("Trial match successfully written to file.");
...
pub fn algorithmic_trial_eval_new(trial_directory: &str, move_orders: u8, a_b_depth: u8, time_cap:u8, mut weights: Vec<f32>) {
    println!("{:?}", weights);
    let mut some = 10;
    while some > 0 {
        let mut i: usize = 559;
            ...
            if weights[f]!=0.0 && weights[f]!= -0.0{
                ...
                for path in paths {
                    if path.is_ok() {
                        ...
                        for algorithmic_pair in &eval_pairs{
                            let tc = TestConfigurationAdj{attacker_eval: algorithmic_pair.0, defender_eval: algorithmic_pair.1, attacker_mo: move_orders, defender_mo: move_orders, a_b_depth};
                            if let Ok(test_results) = trial_play_adj_new(&trial.path(), &tc, &time_cap, weights.clone()) {
                                //Test concluded, write the returned data to our file
                                let mut win = 0;
                                let mut loss = 0;
...
fn trial_play_adj(board_path: &PathBuf, tc: &TestConfigurationAdj, time_cap:&u8) -> Result<TestData, ()> {
    //Silent gameplay for testing algorithms
    let mut instance: GameState = utility::read_state_from_file(board_path).unwrap();
    let mut defender_states: VecDeque<HashMap<u8, Piece>> = VecDeque::new();
    let mut attacker_states: VecDeque<HashMap<u8, Piece>> = VecDeque::new();
    ...
    loop{
        ...
        if turn_parity {
            //Attacker Turn
            let start_time = Instant::now();
            new_move = player::get_move_adj(&instance, &tc.attacker_eval, player_history, tc.attacker_mo, tc.a_b_depth, time_cap, Instant::now());
            let total_time = start_time.elapsed().as_millis();
            if new_move.is_none() {
                //Could be a rare type of Game Victory
                if extinction_check(&instance) {
            ...
            //Defender Turn
            let start_time = Instant::now();
            new_move = player::get_move_adj(&instance, &tc.defender_eval, player_history, tc.defender_mo, tc.a_b_depth, time_cap, Instant::now());
            let total_time = start_time.elapsed().as_millis();
            if new_move.is_none() {
                //Error handling for rare game conditions
                if extinction_check(&instance) {
...
fn trial_play_adj_new(board_path: &PathBuf, tc: &TestConfigurationAdj, time_cap:&u8, weights: Vec<f32>) -> Result<TestData, ()> {
    //Silent gameplay for testing algorithms
    let mut instance: GameState = utility::read_state_from_file(board_path).unwrap();
    let mut defender_states: VecDeque<HashMap<u8, Piece>> = VecDeque::new();
    let mut attacker_states: VecDeque<HashMap<u8, Piece>> = VecDeque::new();
    ...
    loop{
        ...
        if turn_parity {
            //Attacker Turn
            let start_time = Instant::now();
            new_move = player::get_move_adj_new(&instance, &tc.attacker_eval, player_history, tc.attacker_mo, tc.a_b_depth, time_cap, Instant::now(), weights.clone());
            let total_time = start_time.elapsed().as_millis();
            if new_move.is_none() {
                //Could be a rare type of Game Victory
                if extinction_check(&instance) {
            ...
            //Defender Turn
            let start_time = Instant::now();
            new_move = player::get_move_adj_new(&instance, &tc.defender_eval, player_history, tc.defender_mo, tc.a_b_depth, time_cap, Instant::now(), weights.clone());
            let total_time = start_time.elapsed().as_millis();
            if new_move.is_none() {
                //Error handling for rare game conditions
                if extinction_check(&instance) {
