...
pub fn get_move_for_sign_and_relevancy_testing(present:&GameState, eval: &u16, history: &VecDeque<HashMap<u8, Piece>>, move_order: u8, a_b_depth: u8, time_cap: &u8, start_time: Instant)-> Option<MoveRequest>{
    //Alpha-Beta algorithmic players recieve a GameState, and return their favorite.

    let alpha = i32::MIN;
    let beta = i32::MAX;

    a_b_search_for_sign_and_relevancy_testing(present.clone(), a_b_depth, alpha, beta, None, eval, Some(history), move_order, time_cap, start_time).1
}
...
fn a_b_search_for_sign_and_relevancy_testing(state: GameState, depth: u8, alph: i32, bet: i32, path: Option<MoveRequest>, eval: &u16, history: Option<&VecDeque<HashMap<u8, Piece>>>, move_order:u8, time_cap: &u8, start_time: Instant) -> (i32, Option<MoveRequest>) {
    ...
    if maximizing {
        ...
        for possible_move in all_moves {
            ...
            let resultant_state = state.fq_game_update(&possible_move.clone());
            if history.is_some() && history.unwrap().contains(&resultant_state.board) {continue}//Disallow loops (within reason, checking hashmaps is expensive)
            let search_result = a_b_search_for_sign_and_relevancy_testing(resultant_state, depth - 1, alpha, bet, Some(possible_move), eval, None, move_order, time_cap, start_time).0;
            if search_result > value {
                //Update the value and save the relevant move as well
                value = search_result;
                candidate_move = backup;
        ...
        for possible_move in move_list(&state, move_order) {
            ...
            let resultant_state = state.fq_game_update(&possible_move.clone());
            if history.is_some() && history.unwrap().contains(&resultant_state.board) {continue}
            let search_result = a_b_search_for_sign_and_relevancy_testing(resultant_state, depth - 1, alph, beta, Some(possible_move), eval, None, move_order, time_cap, start_time).0;
            if search_result < value {
                value = search_result;
                candidate_move = backup;
            }
...
pub fn get_move_for_weight_testing(present:&GameState, eval: &u16, history: &VecDeque<HashMap<u8, Piece>>, move_order: u8, a_b_depth: u8, time_cap: &u8, start_time: Instant, weights: Vec<f32>)-> Option<MoveRequest>{
    ...
    a_b_search_for_weight_testing(present.clone(), a_b_depth, alpha, beta, None, eval, Some(history), move_order, time_cap, start_time, weights).1
}
...
fn a_b_search_for_weight_testing(state: GameState, depth: u8, alph: i32, bet: i32, path: Option<MoveRequest>, eval: &u16, history: Option<&VecDeque<HashMap<u8, Piece>>>, move_order:u8, time_cap: &u8, start_time: Instant, weights: Vec<f32>) -> (i32, Option<MoveRequest>) {
    ...
    if maximizing {
        ...
        for possible_move in all_moves {
            ...
            let resultant_state = state.fq_game_update(&possible_move.clone());
            if history.is_some() && history.unwrap().contains(&resultant_state.board) {continue}//Disallow loops (within reason, checking hashmaps is expensive)
            let search_result = a_b_search_for_weight_testing(resultant_state, depth - 1, alpha, bet, Some(possible_move), eval, None, move_order, time_cap, start_time,weights.clone()).0;
            if search_result > value {
                //Update the value and save the relevant move as well
                value = search_result;
                candidate_move = backup;
        ...
        for possible_move in move_list(&state, move_order) {
            ...
            let resultant_state = state.fq_game_update(&possible_move.clone());
            if history.is_some() && history.unwrap().contains(&resultant_state.board) {continue}
            let search_result = a_b_search_for_weight_testing(resultant_state, depth - 1, alph, beta, Some(possible_move), eval, None, move_order, time_cap, start_time,weights.clone()).0;
            if search_result < value {
                value = search_result;
                candidate_move = backup;
            }
